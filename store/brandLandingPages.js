...
export const actions = {
    update ({dispatch, commit, state}, {data, id}) {
        const url = `/brandLandingPages/${id}`
    ...
    load ({dispatch, commit, state}, {id = null, force = true, options = {}}) {
        ...
        if (id === null) {
            return dispatch('api/get', {url: `/api/brandLandingPages`, options, debug: false}, root)
                .then(res => {
                    commit('setList', res.data)
                    return res
                })
        } else {
            return dispatch('api/get', {url: `/api/brandLandingPages/{id}`, options}, root)
                .then(res => {
